...
function AgendamentoStart() {
    ...
    $("#txtEspecialidadeAgendamento").change(function () {
        GetCombo('MEDICO', $('.slcMedico'), this.value);
    });

    $("#txtMedicoAgendamento").change(function () {
        GetData($('#txtDataAgendamento'), this.value);
    });
    ...
    $("#txtNomeAgendamento").autocomplete({
        ...
        focus: function (event, ui) {
            $("#txtNomeAgendamento").val(ui.item.nome);
            return false;
        },
...
function validarCampos() {
    ...
    return agendamentoKO.nomePaciente().length > 0 
};

function agendamentoData() {
    this.dataHora = ko.observable("");
    this.data = ko.observableArray([]).extend({ ValidaCadastro: null });
    this.hora = ko.observableArray([]);
    ...
    $.ajax({
        ...
        success: function (data, ajaxOptions, response) {
            ...
            if (data) {
                if (data.value) {
                    if (data.value.length > 0) {
                        agendamentoKO.data(moment(data.value[0].data).format('DD/MM/YYYY hh:mm:ss'));
...
function AddUpdateAgendamento(acao, agendamentoId) {
    ...
    dataValue.agendamento.data = agendamentoKO.dataHora().DataHora;
    dataValue.agendamento.hora = agendamentoKO.dataHora().Hora;
...
function DeleteAgendamento(data) {
    ...
    if (agendamentoKO.tipoUsuario() == 2 || agendamentoKO.tipoUsuario() == 4) {
        AlertServerError('Usuário não autorizado!');
    } else {
        var dataValue = new agendamentoRequest();
...
function GetData(nomeCombo, medicoId) {

    var dataValue = new agendamentoRequest();

    dataValue.agendamento.medicoId = medicoId;
