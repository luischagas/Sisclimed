...
function medicoData() {
    ...
    this.seg = ko.observable(false);
    this.ter = ko.observable(false);
    this.qua = ko.observable(false);
    this.qui = ko.observable(false);
    this.sex = ko.observable(false);
    this.turnoSeg = ko.observable("");
    this.turnoTer = ko.observable("");
    this.turnoQua = ko.observable("");
    this.turnoQui = ko.observable("");
    this.turnoSex = ko.observable("");
            ...
            if (data) {
                if (data.value) {
                    if (data.value.length > 0) {
                        ...
                        medicoKO.turnoSeg(data.value[0].turnoSeg);
                        medicoKO.turnoTer(data.value[0].turnoTer);
                        medicoKO.turnoQua(data.value[0].turnoQua);
                        medicoKO.turnoQui(data.value[0].turnoQui);
                        medicoKO.turnoSex(data.value[0].turnoSex);
                        if (medicoKO.turnoSeg() != '') {
                            medicoKO.seg(true);
                        }
                        if (medicoKO.turnoTer() != '') {

                            medicoKO.ter(true);
                        } 
                        if (medicoKO.turnoQua() != '') {
                            
                           medicoKO.qua(true);
                        }
                        if (medicoKO.turnoQui() != '') {
                           
                           medicoKO.qui(true);
                        }
                        if (medicoKO.turnoSex() != '') {
                           
                           medicoKO.sex(true);
                        }

                        $('#jornada').prop("disabled", true);
                        //$('#chkTer').prop("disabled", true);
                        //$('#chkQua').prop("disabled", true);
                        //$('#chkQui').prop("disabled", true);
                        //$('#chkSex').prop("disabled", true);
                        //$('#optSeg').prop("disabled", true);
                        //$('#optTer').prop("disabled", true);
                        //$('#optQua').prop("disabled", true);
                        //$('#optQui').prop("disabled", true);
                        //$('#optSex').prop("disabled", true);
...
function AddUpdateMedico(acao, MedicoID,tipo) {
    ...
    dataValue.medico.tipo = acao;
    dataValue.medico.medicoId = MedicoID;
    if (medicoKO.seg()) {
        dataValue.medico.turnoSeg = medicoKO.turnoSeg();
    } else {
        dataValue.medico.turnoSeg = "";
}
    if (medicoKO.ter()) {
        dataValue.medico.turnoTer = medicoKO.turnoTer();
    } else {
        dataValue.medico.turnoTer = "";
    }
    if (medicoKO.qua()) {
        dataValue.medico.turnoQua = medicoKO.turnoQua();
    } else {
        dataValue.medico.turnoQua = "";
    }
    if (medicoKO.qui()) {
        dataValue.medico.turnoQui = medicoKO.turnoQui();
    } else {
        dataValue.medico.turnoQui = "";
    }
    if (medicoKO.sex()) {
        dataValue.medico.turnoSex = medicoKO.turnoSex();
    } else {
        dataValue.medico.turnoSex = "";
    }
